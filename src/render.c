...
/*   Updated: 2024/06/05 14:48:27 by jrocha-v         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_textured_wall(t_data *data, int i, int j)
{
	...
	if (!data->rays[i].hit_vert && data->rays[i].is_up)
		put_pixel(&data->img, i, j, get_texture_pixel(&data->txts_img[NO],
				(int)data->rays[i].x_hit % 64, rel_pos));
	else if (!data->rays[i].hit_vert && !data->rays[i].is_up)
		put_pixel(&data->img, i, j, get_texture_pixel(&data->txts_img[SO],
				(int)data->rays[i].x_hit % 64, rel_pos));
	else if (data->rays[i].hit_vert && data->rays[i].is_right)
		put_pixel(&data->img, i, j, get_texture_pixel(&data->txts_img[EA],
				(int)data->rays[i].y_hit % 64, rel_pos));
	else
		put_pixel(&data->img, i, j, get_texture_pixel(&data->txts_img[WE],
				(int)data->rays[i].y_hit % 64, rel_pos));
}
